...
function calculate(){
  ...
  let shiping_amount = $('#trans_total_price').text();
  console.log(shiping_amount)
  if(shiping_amount == ""){
    console.log("empty shipping charges")
  }
  let total_ship_t = parseInt(total_price) + parseFloat(shiping_amount,10)
  $('#total_price').text(total_price.toFixed(2));
  $('#total_tax').text(total_tax.toFixed(2)*2+"  %  "+TAX_RATE);
  $('#t_t_total_price').text(total_ship_t.toFixed(2));

}


var newRow = '<tr><td><a class="control removeRow" href="#">x</a><span contenteditable>Item</span></td><td><span contenteditable>Description</span></td><td><span contenteditable>HSN Code</span></td><td class="daterelated"><span contenteditable>Date</span></td><td class="amount"><input type="text" value="1"/></td><td class="unit"><input type="text" value="kg"/></td><td class="rate"><input type="text" value="60" /></td><td class="tax taxrelated"></td><td class="sum"></td></tr>';

$('.invoicelist-body').on('keyup','input',function(){
  calculate();
});

$('.invoicelist-footer').on('keyup','.trans_total_price',function(){
  calculate();
});
